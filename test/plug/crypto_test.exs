defmodule Plug.CryptoTest do
  ...
  describe "sign and verify" do
    ...
    test "supports max age in seconds" do
      ...
      assert verify(@key, "id", token, max_age: 0) == {:error, :expired}
  ...
  describe "encrypt and decrypt" do
    ...
    test "fails on missing token" do
      assert decrypt(@key, "secret", "id", nil) == {:error, :missing}
    end

    test "fails on invalid token" do
      token = encrypt(@key, "secret", "id", 1)

      assert decrypt(@key, "secret", "id", "garbage") ==
               {:error, :invalid}

      assert decrypt(@key, "secret", "not_id", token) ==
               {:error, :invalid}

      assert decrypt(@key, "not_secret", "id", token) ==
               {:error, :invalid}
    end
    ...
    test "supports max age in seconds" do
      ...
      assert decrypt(@key, "secret", "id", token, max_age: 0) == {:error, :expired}
    ...
    test "supports :infinity for max age" do
      token = encrypt(@key, "secret", "id", 1)
      assert decrypt(@key, "secret", "id", token, max_age: :infinity) == {:ok, 1}
    end

    test "supports signed_at in seconds" do
      seconds_in_day = 24 * 60 * 60
      day_ago_seconds = System.system_time(:second) - seconds_in_day
      token = encrypt(@key, "secret", "id", 1, signed_at: day_ago_seconds)
      assert decrypt(@key, "secret", "id", token, max_age: seconds_in_day + 1) == {:ok, 1}
    ...
    test "passes key_iterations options to key generator" do
      signed1 = encrypt(@key, "secret", "id", 1, signed_at: 0, key_iterations: 1)
      signed2 = encrypt(@key, "secret", "id", 1, signed_at: 0, key_iterations: 2)
      assert signed1 != signed2
    end

    test "passes key_digest options to key generator" do
      signed1 = encrypt(@key, "secret", "id", 1, signed_at: 0, key_digest: :sha256)
      signed2 = encrypt(@key, "secret", "id", 1, signed_at: 0, key_digest: :sha512)
      assert signed1 != signed2
    end

    test "passes key_length options to key generator" do
      signed1 = encrypt(@key, "secret", "id", 1, signed_at: 0, key_length: 16)
      signed2 = encrypt(@key, "secret", "id", 1, signed_at: 0, key_length: 32)
      assert signed1 != signed2
    end
  end
end
